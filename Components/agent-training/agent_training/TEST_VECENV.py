...
def train_agent(exp_path,model,env,testing_interval,max_steps,model_type,basicdate,tb_writer,tb_log_name,early_stopping=True, previous_steps=0):
    ...
    while steps < max_steps:
        ...
        if early_stopping:
            victory_count, avg_reward = helper.check_victory(model,env,trials = 10,report_separate=True)
            print(victory_count)
            print(avg_reward)
...
def run_self_play(exp_name,exp_path,basicdate,
                    ...
                    model_type='PPO2',log_to_tb=False,image_based=True,num_parallel_envs=5):
    pelican_training_steps = 0
    panther_training_steps = 0
    config_path = 'C:\\Users\Jacopo\Documents\plark_ai_public\Components\plark-game\plark_game\game_config\10x10\balanced.json'
    ...
    if parallel:
        pelican_env = SubprocVecEnv([lambda:PlarkEnv(driving_agent='pelican',config_file_path='C:/Users/Jacopo/Documents/plark_ai_public/Components/plark-game/plark_game/game_config/10x10/pelican_easy.json',image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3) for _ in range(num_parallel_envs)])
    else:
        pelican_env = PlarkEnv(driving_agent='pelican',config_file_path='C:/Users/Jacopo/Documents/plark_ai_public/Components/plark-game/plark_game/game_config/10x10/pelican_easy.json',image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3)
    ...
    if parallel:
        panther_env = SubprocVecEnv([lambda:PlarkEnv(driving_agent='panther',pelican_agent_filepath=pelican_agent_filepath,config_file_path=config_path,image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3) for _ in range(num_parallel_envs)])
    else:
        panther_env = PlarkEnv(driving_agent='panther',pelican_agent_filepath=pelican_agent_filepath,config_file_path=config_path,image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3)
    ...
    for i in range(self_play_iterations):
        ...
        if parallel:
            pelican_env = SubprocVecEnv([lambda:PlarkEnvSparse(driving_agent='pelican',panther_agent_filepath=panther_agent_filepath,config_file_path=config_path,image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3) for _ in range(num_parallel_envs)])
        else:
            pelican_env = PlarkEnvSparse(driving_agent='pelican',panther_agent_filepath=panther_agent_filepath,config_file_path=config_path,image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3)
        ...
        if parallel:
            panther_env = SubprocVecEnv([lambda:PlarkEnvSparse(driving_agent='panther',pelican_agent_filepath=pelican_agent_filepath,config_file_path=config_path,image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3) for _ in range(num_parallel_envs)])
        else:
            panther_env = PlarkEnvSparse(driving_agent='panther',pelican_agent_filepath=pelican_agent_filepath,config_file_path=config_path,image_based=image_based,random_panther_start_position=True,max_illegal_moves_per_turn=3)
...
if __name__ == '__main__':
    ...
    run_self_play(exp_name,exp_path,basicdate,
                    ...
                    self_play_testing_interval=10,self_play_max_learning_steps_per_agent=50,self_play_iterations=20,
