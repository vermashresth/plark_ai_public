...
class Newgame:
    ...
    def __init__(self, game_config, **kwargs):
        ...
        # Load agents
        relative_basic_agents_filepath = os.path.join(
            os.path.dirname(__file__), self.relative_basic_agents_filepath
        )
    ...
    def set_pelican(self, pelican):
        ...
        self.pelicanAgent = Pelican_Agent_Set_Agent(
            pelican, Observation(self, **kwargs)
        )

    def set_panther(self, panther):
        kwargs = {}
        kwargs["driving_agent"] = "panther"
        self.pantherAgent = Panther_Agent_Set_Agent(
            panther, Observation(self, **kwargs)
        )
            ...
            if self.driving_agent == "pelican":
                ...
                if (
                    ...
                    if (
                        ...
                        return self.gameState, self.gameBoard.UIOutput(
                            self.gamePlayerTurn
                        )
            else:
                self.pelicanPhase()
    ...
    def perform_panther_action(self, panther_action):
        ...
        if panther_action in ["1", "2", "3", "4", "5", "6"]:
            new_col, new_row = self.gameBoard.getNeighbours(
                int(panther_action), self.pantherPlayer.col, self.pantherPlayer.row
            )
            ...
            if self.gameBoard.withinMap(new_col, new_row):
                ...
                self.update_status_bar(
                    "Illegal panther move : Moving outside of game map", "red"
                )
    ...
    def perform_pelican_action(self, pelican_action):
        if pelican_action in ["1", "2", "3", "4", "5", "6"]:
            new_col, new_row = self.gameBoard.getNeighbours(
                int(pelican_action), self.pelicanPlayer.col, self.pelicanPlayer.row
            )
            ...
            if list(
                filter(
                    lambda item: (item.type == "SONOBUOY"), self.pelicanPlayer.payload
                )
            ):

                # check if current hex already contains a sonobuoy
                if self.gameBoard.is_item_type_in_cell(
                    "SONOBUOY", self.pelicanPlayer.col, self.pelicanPlayer.row
                ):
            ...
            if list(
                filter(
                    lambda item: (item.type == "TORPEDO"), self.pelicanPlayer.payload
                )
            ):

                # check if current hex already contains a torpedo
                if self.gameBoard.is_item_type_in_cell(
                    "TORPEDO", self.pelicanPlayer.col, self.pelicanPlayer.row
                ):
    ...
    def madmanPhase(self, pelicanMoves):
        ...
        for pelicanMove in pelicanMoves:
            ...
            if (
                len(
                    self.gameBoard.searchRadius(
                        pelicanMove["col"],
                        pelicanMove["row"],
                        self.pelican_parameters["madman_range"],
                        "PANTHER",
                    )
                )
                > 0
    ...
    def maypolePhase(self):
        for bouy in self.globalSonobuoys:
            if (
                len(
                    self.gameBoard.searchRadius(
                        bouy.col, bouy.row, bouy.range, "PANTHER"
                    )
                )
                > 0
            ):
    ...
    def pantherPhase(self):
        ...
        while True:
            ...
            if (
                self.gameState == "ESCAPE"
                or self.panther_move_in_turn >= self.panther_parameters["move_limit"]
    ...
    def bloodhoundPhase(self):
        ...
        for t in self.globalTorps:
            ...
            # results.
            detected = self.gameBoard.searchRadius(
                t.col, t.row, t.searchRadius, ["PANTHER", "TORPEDO"]
            )
            detected.remove(t)
            ...
            if len(detected) > 0:
                ...
                detected.sort(
                    key=lambda result: self.gameBoard.distance(
                        t.col, t.row, result.col, result.row
                    )
                )
                ...
                if self.torpedo_parameters["hunt"]:
                    ...
                    path = self.gameBoard.get_path(
                        t.col, t.row, detected[0].col, detected[0].row
                    )
                ...
                if detected[0].type == "PANTHER" and (
                    (detected[0].col == t.col) and (detected[0].row == t.row)
                ):
        ...
        if self.winchester_rule:
            if self.gameState == "Running":
                undeployed_torpedoes = len(
                    list(
                        filter(
                            lambda item: (item.type == "TORPEDO"),
                            self.pelicanPlayer.payload,
                        )
                    )
                )
    ...
    def update_status_bar(self, message, colour):
        self.status_bar = {"message": message, "fill": colour}

    def clear_status_bar(self):
        self.status_bar = {"message": "", "fill": None}

    def _state(self, view):
        if view not in ["PANTHER", "PELICAN", "ALL"]:
            raise ValueError(
                "View", view, " is not one of:", str(["PANTHER", "PELICAN", "ALL"])
            )
        ...
        if (
            view in ["PELICAN", "ALL"]
            or self.driving_agent == "pelican"
            or self.output_view_all
        ):
            ...
            state["remaining_sonobuoys"] = len(
                [obj for obj in self.pelicanPlayer.payload if obj.type == "SONOBUOY"]
            )
            state["deployed_sonobuoys"] = self.globalSonobuoys
            state["remaining_torpedoes"] = len(
                [obj for obj in self.pelicanPlayer.payload if obj.type == "TORPEDO"]
            )
    ...
    def load_configurations(self, game_config, **kwargs):
        ...
        # Game settings
        self.maxTurns = kwargs.get(
            "maximum_turns", game_config["game_settings"]["maximum_turns"]
        )
        self.map_width = kwargs.get(
            "map_width", game_config["game_settings"]["map_width"]
        )
        self.map_height = kwargs.get(
            "map_height", game_config["game_settings"]["map_height"]
        )
        self.driving_agent = kwargs.get(
            "driving_agent", game_config["game_settings"]["driving_agent"]
        )
        self.max_illegal_moves_per_turn = kwargs.get(
            "max_illegal_moves_per_turn",
            game_config["game_settings"]["max_illegal_moves_per_turn"],
        )

        # Render settings
        self.hexScale = kwargs.get(
            "hex_scale", game_config["render_settings"]["hex_scale"]
        )
        self.output_view_all = kwargs.get(
            "output_view_all", game_config["render_settings"]["output_view_all"]
        )
        ...
        # Game Rules
        self.bingo_limit = kwargs.get(
            "bingo_limit", game_config["game_rules"]["bingo_limit"]
        )
        self.winchester_rule = kwargs.get(
            "winchester_rule", game_config["game_rules"]["winchester_rule"]
        )
        self.escape_rule = kwargs.get(
            "escape_rule", game_config["game_rules"]["escape_rule"]
        )

        # Pelican settings
        # Are the start positions read in from the json here?
        self.pelican_start_col = kwargs.get(
            "pelican_start_col",
            game_config.get("game_rules", {})
            .get("pelican", {})
            .get("start_col", int(self.map_width / 2)),
        )
        self.pelican_start_row = kwargs.get(
            "pelican_start_row",
            game_config.get("game_rules", {}).get("pelican", {}).get("start_row", 0),
        )
        self.pelican_parameters = {
            "move_limit": kwargs.get(
                "pelican_move_limit", game_config["game_rules"]["pelican"]["move_limit"]
            ),
            "agent_filepath": kwargs.get(
                "pelican_agent_filepath",
                game_config["game_rules"]["pelican"]["agent_filepath"],
            ),
            "agent_name": kwargs.get(
                "pelican_agent_name", game_config["game_rules"]["pelican"]["agent_name"]
            ),
            "madman_range": kwargs.get(
                "madman_range", game_config["game_rules"]["pelican"]["madman_range"]
            ),
            "default_torps": kwargs.get(
                "default_torps", game_config["game_rules"]["pelican"]["default_torps"]
            ),
            "default_sonobuoys": kwargs.get(
                "default_sonobuoys",
                game_config["game_rules"]["pelican"]["default_sonobuoys"],
            ),
            "render_height": kwargs.get(
                "pelican_render_height",
                game_config["game_rules"]["pelican"]["render_height"],
            ),
            "render_width": kwargs.get(
                "pelican_render_width",
                game_config["game_rules"]["pelican"]["render_width"],
            ),
        }

        # Torpedo settings
        self.torpedo_parameters = {
            "turn_limit": kwargs.get(
                "torpedos_turn_limit",
                game_config["game_rules"]["torpedo"]["turn_limit"],
            ),
            "hunt": kwargs.get(
                "torpedos_hunt", game_config["game_rules"]["torpedo"]["hunt"]
            ),
            "speed": kwargs.get(
                "torpedos_speed", game_config["game_rules"]["torpedo"]["speed"]
            ),
            "search_range": kwargs.get(
                "torpedos_search_range",
                game_config["game_rules"]["torpedo"]["search_range"],
            ),
        }

        # Sonobouy settings
        self.sonobuoy_parameters = {
            "active_range": kwargs.get(
                "sonobuoy_active_range",
                game_config["game_rules"]["sonobuoy"]["active_range"],
            ),
            "display_range": kwargs.get(
                "display_range", game_config["game_rules"]["sonobuoy"]["display_range"]
            ),
        }

        # Panther settings
        self.panther_start_col = kwargs.get(
            "panther_start_col",
            game_config.get("game_rules", {})
            .get("panther", {})
            .get("start_col", int(self.map_width / 2)),
        )
        self.panther_start_row = kwargs.get(
            "panther_start_row",
            game_config.get("game_rules", {})
            .get("panther", {})
            .get("start_row", int(self.map_height - 5)),
        )

        self.panther_parameters = {
            "move_limit": kwargs.get(
                "panther_move_limit", game_config["game_rules"]["panther"]["move_limit"]
            ),
            "agent_filepath": kwargs.get(
                "panther_agent_filepath",
                game_config["game_rules"]["panther"]["agent_filepath"],
            ),
            "agent_name": kwargs.get(
                "panther_agent_name", game_config["game_rules"]["panther"]["agent_name"]
            ),
            "render_height": kwargs.get(
                "panther_render_height",
                game_config["game_rules"]["panther"]["render_height"],
            ),
            "render_width": kwargs.get(
                "panther_render_width",
                game_config["game_rules"]["panther"]["render_width"],
            ),
        }
    ...
    def create_game_objects(self):
        ...
        self.gameBoard.addItem(
            self.pantherPlayer.col, self.pantherPlayer.row, self.pantherPlayer
        )
        self.gameBoard.addItem(
            self.pelicanPlayer.col, self.pelicanPlayer.row, self.pelicanPlayer
        )
    ...
    def default_game_variables(self):
        ...
        self.illegal_pelican_move = False
        self.illegal_panther_move = False
        self.status_bar = {"message": "", "fill": "red"}

    def load_pelican_using_path(
        self, file_path, observation=None, image_based=False, **kwargs
    ):
        ...
        self.pelicanAgent = Pelican_Agent_Load_Agent(
            agent_filepath, metadata["algorithm"], observation, image_based
        )

    def load_panther_using_path(
        self, file_path, observation=None, image_based=False, **kwargs
    ):
        metadata_filepath = os.path.join(file_path, "metadata.json")
        agent_filepath = glob.glob(file_path + "/*.zip")[0]
...
def load_agent(
    file_path, agent_name, basic_agents_filepath, game, in_tournament=False, **kwargs
):
    if (
        ".py" in file_path
    ):  # This is the case for an agent in a non standard location or selected throguh web ui.
        # load python
        ...
        for f in files:
            if ".zip" not in f:
                ...
                image_based = True
                if (
                    "image_based" in metadata and metadata["image_based"] == False
                ):  # If the image_based flag is not present asume image based, if it is there and set to false the set to false.
                    image_based = False
