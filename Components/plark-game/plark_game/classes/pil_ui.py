...
class HexagonalGrid:
    ...
    def __init__(self, scale, rows, cols, *args, **kwargs):
        ...
        self.cols = cols
        self.width = int(
            ((self.scale + 1) * self.cols) - ((self.scale / 4) * self.cols) + scale
        )
        self.height = int((self.scale * self.rows) + ((self.scale / 2) + self.scale))
    ...
    def setCell(self, idraw, xCell, yCell, items, *args, **kwargs):
        ...
        pix_x = (
            (self.hexaSize - (self.hexaSize / 4) + 1) * xCell
        ) + self.margin  # the +1 in this line adds horizontal  spacing between the cells
    ...
    def createPelican(self, x, y):
        ...
        self.idraw.polygon(
            [point1, point2, point3], fill=(255, 255, 255), outline=(255, 255, 255)
        )
    ...
    def create_torpedo(self, x, y):
        ...
        self.idraw.rectangle([point1, point2], outline=(0, 0, 0), fill=(0, 0, 0))
    ...
    def create_explosion(self, x, y):
        ...
        self.idraw.polygon([point1, point2, point3, point4, point5, point6], fill="red")
    ...
    def create_hexagone(self, idraw, x, y, items, fill, outline=None):
        ...
        self.idraw.polygon(
            [point1, point2, point3, point4, point5, point6], fill=fill, outline=outline
        )
        ...
        if "SONOBUOY" in items:
            self.create_sonobuoy(
                x - (quarter + quarter / 2), y - (quarter / 2), items["SONOBUOY"]
            )
...
class Loadout:
    def __init__(
        self, loadout, location, font, scale, sonobuoy_range, torp_hunt, torp_speeds
    ):
        self.font = font
        ...
        self.sonobuoy_count = len(
            list(filter(lambda item: (item.type == "SONOBUOY"), loadout))
        )
        self.torp_count = len(
            list(filter(lambda item: (item.type == "TORPEDO"), loadout))
        )
        self.madman_loadout_location = [self.location[0], self.location[1]]
        ...
        self.sonobuoy_loadout_location = [
            ...
            self.torpedo_loadout_location[1]
            + (self.scale * math.ceil(self.torp_count / self.loadout_cols))
            + (self.scale * 0.5)
            + self.scale
            + (self.scale * len(self.torp_speeds) + 1),
        ]

    def render(self, loadout, madman, idraw):
        self.torpedo_list = list(filter(lambda item: (item.type == "TORPEDO"), loadout))
    ...
    def render_madman_loadout(self, location, idraw):
        ...
        madman_text_location = [location[0], location[1]]
        ...
        idraw.rectangle(
            [madman_location_point1, madman_location_point2], outline=fill, fill=fill
        )
    ...
    def render_sonobuoy_loadout(self, location, idraw):
        ...
        sonobuoy_range_location = [
            location[0],
            location[1]
            + ((self.scale * self.sonobuoy_count) / self.loadout_cols)
            + (self.scale * 0.5),
        ]
        ...
        idraw.text(
            sonobuoy_range_location, sonobuoy_range_string, fill="black", font=self.font
        )
        ...
        for i in range(len(self.sonobuoy_list)):
            ...
            sb_location = (
                sonobuoy_bay[0] + (bayCount * self.scale / 1.5),
                sonobuoy_bay[1],
            )
        ...
        for i in range(0, self.sonobuoy_range):
            ...
            pix_x = (
                ((hex_size - (hex_size / 4)) * i)
                + hex_size / 2
                + sonobuoy_range_location[0]
            )
    ...
    def render_torpedo_loadout(self, location, idraw):
        ...
        # torp hunt icon
        self.render_torpedo(
            hunt_icon_location, idraw, torp_colour, torp_colour
        )  # sets both fill and outline to torp colour
...
class Counters:
    def __init__(self, maxturn, location, font):
        ...
        self.turn_location = [self.location[0], self.location[1]]
        self.move_location = [self.location[0], self.location[1] + self.font.size]
    ...
    def renderCounters(self, turn, move, max_moves, state, idraw):
        ...
        idraw.text(self.turn_location, turn_string, fill="black", font=self.font)
        idraw.text(self.move_location, move_string, fill="black", font=self.font)
        idraw.text(
            self.game_state_location, game_state_string, fill="black", font=self.font
        )
...
class Status_Bar:
    ...
    def render_status_bar(self, status_Bar, idraw):
        ...
        if message != "":
            ...
            idraw.rectangle([self.point1, self.point2], outline=fill, fill=fill)
            idraw.text(
                self.status_bar_text_location, message, fill="white", font=self.font
            )
...
class PIL_UI:
    def __init__(
        ...
        self.status_bar = Status_Bar(
            self.status_bar_location, self._width, self.font, self.scale
        )
    ...
    def update(self, gameFile, render_width=None, render_height=None):
        ...
        if gameFile["driving_agent"] == "pelican" or self.render_all:
            ...
            self.loadout.render(
                gameFile["pelican_loadout"], gameFile["madman"], self.idraw
            )
        elif gameFile["driving_agent"] == "panther":
            self.counters.renderCounters(
                gameFile["turn_count"],
        ...
        if render_height is not None and render_width is not None:
            ...
            return self.ui_img.resize(
                [self.render_width, self.render_height], Image.LANCZOS
            )
