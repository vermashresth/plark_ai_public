...
class Observation:
    def __init__(self, game, **kwargs):
        ...
        assert (
            game.map_width <= self.max_grid_width
        ), "Game width must not be greater than max grid width: {}".format(
            self.max_grid_width
        )
        assert (
            game.map_height <= self.max_grid_height
        ), "Game height must not be greater than max grid height: {}".format(
            self.max_grid_width
        )
        ...
        assert (
            game.maxTurns <= self.max_turns
        ), "Game max turns must not be greater than max possible turns: {}".format(
            self.max_turns
        )
        ...
        if self.driving_agent == "pelican":
            ...
            assert (
                game.pelican_parameters["move_limit"] <= self.max_pelican_moves
            ), "Game move limit must not be greater than max move limit: {}".format(
                self.max_pelican_moves
            )
            ...
            assert (
                game.sonobuoy_parameters["active_range"] <= self.max_sb_range
            ), "Sonobuoy active range must not be greater than max range: {}".format(
                self.max_sb_range
            )
            ...
            assert (
                game.pelican_parameters["default_sonobuoys"] <= self.max_sonobuoys
            ), "Starting number of sonobuoys must not be greater than maximum possible sonobuoys: {}".format(
                self.max_sonobuoys
            )
            ...
            assert (
                game.pelican_parameters["default_torps"] <= self.max_torpedoes
            ), "Starting number of sonobuoys must not be greater than maximum possible sonobuoys: {}".format(
                self.max_torpedoes
            )
            ...
            assert (
                game.torpedo_parameters["turn_limit"] <= self.max_torpedo_turns
            ), "Torpedo turn limit must not be greater than the maximum torpedo turns: {}".format(
                self.max_torpedo_turns
            )
            for speed in game.torpedo_parameters["speed"]:
                assert (
                    speed <= self.max_torpedo_speed
                ), "Torpedo speed for each turn must not be greater than the maximum torpedo speed: {}".format(
                    self.max_torpedo_speed
                )
            ...
            # Number of moves remaining per turn
            assert (
                game.panther_parameters["move_limit"] <= self.max_panther_moves
            ), "Game move limit must not be greater than max move limit: {}".format(
                self.max_panther_moves
            )
    ...
    def get_observation(self, state):
        ...
        new_pelican_col, new_pelican_row = self._get_location(
            self.game.gameBoard, "PELICAN"
        )
        ...
        new_panther_col, new_panther_row = self._get_location(
            self.game.gameBoard, "PANTHER"
        )
        ...
        state["deployed_sonobuoys"] = [
            {"col": b.col, "row": b.row, "state": b.state}
            for b in self.game.globalSonobuoys
        ]
        ...
        state["deployed_torpedoes"] = [
            {"col": t.col, "row": t.row} for t in self.game.globalTorps
        ]
        ...
        if self.driving_agent == "pelican":
            # Remaining moves
            remaining_pelican_moves = (
                state["pelican_max_moves"] - state["pelican_move_in_turn"]
            )
            ...
            # Remaining moves
            remaining_panther_moves = (
                state["panther_max_moves"] - state["panther_move_in_turn"]
            )
