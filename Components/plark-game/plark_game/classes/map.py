...
class HexCell:
    def __init__(self, col, row):
        self.coordinate = {"row": row, "col": col}
...
class Map:
                    ...
                    for item in hexCell.getCellObjects():
                        if item.type == "SONOBUOY":
                            ...
                            for cell in sonar_area:
                                cell_col = cell["col"]
                                cell_row = cell["row"]
                                uiOutput[cell_col][cell_row]["objects"][
                                    "sonar_area"
                                ] = item.state
            ...
            for ny in range(yStart, yEnd):
                ...
                if (nz <= startCube["z"] + r) and (nz >= startCube["z"] - r):
                    ...
                    if self.withinMap(col, row):
                        results.append({"col": col, "row": row})
    ...
    def distance(self, point1_col, point1_row, point2_col, point2_row):
        ...
        return max(
            abs(point1Cube["x"] - point2Cube["x"]),
            abs(point1Cube["y"] - point2Cube["y"]),
            abs(point1Cube["z"] - point2Cube["z"]),
        )
    ...
    def get_edges(self):
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                if ((col == 0) or (col == (self.cols - 1))) or (
                    (row == 0) or (row == (self.rows - 1))
                ):
    ...
    def get_path(
        ...
        class Node:
            def __init__(self, col, row, g, h, parent=None):
                ...
                self.parent = parent
                self.g = (
                    g  # graphical score, the amount of moves from the starting location
                )
                self.h = (
                    h  # heuristic score, the distance from current node to end location
                )
        ...
        while not goal_reached:
            ...
            if (currentNode.col == end_locataion_col) and (
                currentNode.row == end_locataion_row
            ):
                goal_reached = True
                ...
                for n in neighbours:
                    ...
                    if (n["col"] != parent_node.col) or (
                        n["row"] != parent_node.row
                    ):  # test if the neighbour is the parent. the get radius function includes the starting location in its results but we dont want it here.
                        ...
                        current_node_closed = next(
                            (
                                x
                                for x in closed_list
                                if (x.col == n["col"]) and (x.row == n["row"])
                            ),
                            None,
                        )
                        current_node_open = next(
                            (
                                x
                                for x in open_list
                                if (x.col == n["col"]) and (x.row == n["row"])
                        ...
                        if (
                            current_node_closed in closed_list
                            and new_g < current_node_closed.g
                        ):
                            current_node_closed.g = new_g
                            current_node_closed.parent = parent_node

                        elif (
                            current_node_open in open_list
                            and new_g < current_node_open.g
                        ):
                        ...
                        # If the node has not been added to any list create a node and add it to the open list
                            ...
                            current_node_open not in open_list
                            and current_node_closed not in closed_list
                        ):
                            open_list.append(
                                Node(
                                    n["col"],
